...
class CommonSetup(aetest.CommonSetup):
    ...
    def connect_to_devices(self, testbed):
        ...
        stackmd_typ_exp = "Push Stack Mode"
        stktm_typ_exp = "Push Stack Mode Time"
        cncl_stkmd_typ_exp = "Cancel Stack Mode"  
        stackmd_typ_init_exp = "Stack Mode Push Initiated"
        stktm_typ_init_exp = "Stack Switch Time Push Initiated"
        cncl_stkmd_typ_init_exp = "Stack Mode Cancel Initiated"
        stackmd_typ_fin_exp = "Stack Mode Operation Completed"
        stktm_typ_fin_exp = "Stack Switch Time Operation Completed"
        cncl_stkmd_typ_fin_exp = "Stack Mode Cancel Operation Completed"
        pshstack_msg_exp = "Finishing subnet %s after CONFIGURED_STACKMODE"% wisun_exp_subnet1
        pshstack_tme_msg_exp = "Finishing subnet %s after SUCCESS"% wisun_exp_subnet1
        cancel_msg_exp = "Finishing subnet %s after CANCELLED_STACKMODE_SWITCH"% wisun_exp_subnet1
        log_msg_stkmd = "Finishing subnet %s after CONFIGURED_STACKMODE"% wisun_exp_subnet1
        log_msg_skttm = "Finishing subnet %s after SUCCESS"% wisun_exp_subnet1
        log_msg_cncl = "Finishing subnet %s after CANCELLED_STACKMODE_SWITCH"% wisun_exp_subnet1
        ...
        cancel_bfr_stk_exp_str = "Unable to cancel stack mode operation. Reason: Devices in the subnet %s. did not receive stack " \
                                 "mode/time value. Please push stack mode/time and continue." % wisun_exp_subnet1
        ...
        push_stack_tm_active_job = "Unable to push stack mode switch time. Reason: Exception when trying to set stack switch time " \
                                   "for panid [\'%s\'] with an already active stack operation" % cgmesh_pan_id
        ...
        pshstack_ver_chk = "Unable to Switch Wi-SUN Stack. Reason: Devices in following subnets prefix: [%s] spanning across groups: " \
                           "[%s] need to be upgraded to the version equal to 6.2 MR" % (wisun_exp_subnet, def_fw_groupname)
        ...
        self.parent.parameters.update(stackmd_typ_init_exp = stackmd_typ_init_exp)
        self.parent.parameters.update(stktm_typ_init_exp = stktm_typ_init_exp)
        self.parent.parameters.update(cncl_stkmd_typ_init_exp = cncl_stkmd_typ_init_exp)
        self.parent.parameters.update(stackmd_typ_fin_exp = stackmd_typ_fin_exp)
        self.parent.parameters.update(stktm_typ_fin_exp = stktm_typ_fin_exp)
        self.parent.parameters.update(cncl_stkmd_typ_fin_exp = cncl_stkmd_typ_fin_exp)

###################################################################
###                  LOCAL METHODS                              ###
###################################################################

def Push_And_Verify_The_StackMode(driver, pan_id, stk_typ, stk_init_typ, stak_fin_typ, lst_snt_msg, total_node, schd_stk_tm = None):
    """
    ...
    time.sleep(5)
    if stack_status in str(stk_init_typ):
...
def Push_And_Verify_The_StackMode_In_Custom_Group(driver, pan_id, stk_typ, stak_fin_typ, total_node, schd_stk_tm = None):
    """
    Method to Push/Cancel stack mode and verify the Stack operation status, Type, Log message sent and Scheduled stack change status
    :param stk_typ: Push stack type
    :param total_node: Expected success node to verify
...
def Verify_Push_Stackmode_time_State_Changes(driver, cur_group, eids, stk_typ, stk_typ_init, schd_stk_tm):
    """
    ...
    time.sleep(5)
    if stack_status in str(stk_typ_init):
        log.info("##### \'%s\' initiated successfully #####" % stack_status)
        log.info("Click on Refresh button")
        driver.find_element_by_xpath("//div[@id='fwSubnetListPagingToolbar']//table[@class='x-toolbar-ct']//td[@class='x-toolbar-right']"
                "//table[@class='x-toolbar-right-ct']//tr[@class='x-toolbar-right-row']//td//em/button[@type='button']").click()
...
class Tc1_Validate_Cancel_Stackmode_When_Panid_One_In_Wisun_Another_In_CGMesh(aetest.Testcase):
    ...
    def Tc1_Validate_Cancel_Stackmode_When_Panid_One_In_Wisun_Another_In_CGMesh(self, driver, cncl_exp_str, cancel_wisun_exp_str, error_log_cncl_bfr_stkmd_nonwis, 
        ...
        try:
            ...
            log.info("Check for the events dated: %s"% dte) 
...
class Tc3_Validate_Push_Stackmode_Time_When_Panid_One_In_Wisun_Another_In_CGMesh(aetest.Testcase):
    ...
    def Tc3_Validate_Push_Stackmode_Time_When_Panid_One_In_Wisun_Another_In_CGMesh(self, driver, stackmd_exp_str, stktm_bfr_stkmd_exp_str,
        ...
        try:
            ...
            log.info("Check for the events dated: %s"% dt)  
            message_list = [element.text for element in driver.find_elements_by_xpath(
...
class Tc6_Validate_Overall_Stackmode_Mode_Push_And_Active_Running_Job(aetest.Testcase):
    ...
    def Tc6_Validate_Overall_Stackmode_Mode_Push_And_Active_Running_Job(self, driver, cgmesh_pan_id, stackmd_exp_str, stack_node_cnt,
                                                 stackmd_cfm_exp_str, stackmd_typ_exp, stackmd_typ_init_exp, stackmd_typ_fin_exp,
                                                 pshstack_msg_exp, log_msg_stkmd, push_stack_md_active_job):
        try:
            global driver_utils
            global auto_user_ws
            import datetime
            ...
            temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, stackmd_typ_exp, stackmd_typ_init_exp, stackmd_typ_fin_exp, 
                                                  pshstack_msg_exp, stack_node_cnt)   
...
class Tc7_Validate_Overall_Stackmode_Cancel(aetest.Testcase):
    @aetest.test
    def Tc7_Validate_Overall_Stackmode_Cancel(self, driver, cgmesh_pan_id, cncl_exp_str, cancel_exp_str, cncl_stkmd_typ_init_exp, 
                                              cncl_stkmd_typ_fin_exp, stack_node_cnt, cncl_stkmd_typ_exp, cancel_msg_exp, log_msg_cncl):
        try:
            global driver_utils
            global auto_user_ws
            import datetime
            ...
            temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, cncl_stkmd_typ_exp, cncl_stkmd_typ_init_exp, 
                                                cncl_stkmd_typ_fin_exp, cancel_msg_exp, stack_node_cnt)  
...
class Tc8_Validate_Overall_Stacktime_Push_And_Active_Running_Job(aetest.Testcase):
    @aetest.test
    def Tc8_Validate_Overall_Stacktime_Push_And_Active_Running_Job(self, driver, nms_server, ir500_eids, cgmesh_pan_id, stackmd_exp_str, stackmd_typ_init_exp, 
                                                 stackmd_typ_fin_exp, stktm_typ_init_exp, stktm_typ_fin_exp, stack_node_cnt, push_stack_tm_active_job,
        ...
        try:
            ...
            #temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, stktm_typ_exp, stktm_typ_init_exp, stktm_typ_fin_exp, pshstack_tme_msg_exp, stack_node_cnt, config_dt_tm)
            temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, stktm_typ_exp, stktm_typ_init_exp, stktm_typ_fin_exp, 
                                                 pshstack_tme_msg_exp, stack_node_cnt)   
            ...
            log.info("Check for the events dated: %s"% dt1)  
            message_list = [element.text for element in driver.find_elements_by_xpath(
...
class Tc9_Validate_Overall_StackMode_And_Stacktime_Cancel_And_Active_Running_Job(aetest.Testcase):
    ...
    def Tc9_Validate_Overall_StackMode_And_Stacktime_Cancel_And_Active_Running_Job(self, driver, cgmesh_pan_id, cncl_exp_str,
                                                   cncl_stkmd_typ_init_exp, cncl_stkmd_typ_fin_exp,cancel_exp_str, stack_node_cnt, 
                                                   cncl_stkmd_typ_exp, cancel_msg_exp, log_msg_cncl, cancel_stack_active_job):
        try:
            global driver_utils
            global auto_user_ws
            import datetime
...
class Tc11_Validate_Per_Device_Stacktime_State_Changes(aetest.Testcase):
    @aetest.test
    def Tc11_Validate_Per_Device_Stacktime_State_Changes(self, driver, ir500_eids, cgmesh_pan_id, stackmd_exp_str, stktm_typ_exp, stktm_typ_init_exp, log_msg_skttm):
        try:
            global driver_utils
            global auto_user_ws
            import datetime
            ...
            temp = Verify_Push_Stackmode_time_State_Changes(driver, fw_groupname, ir500_eids, stktm_typ_exp, stktm_typ_init_exp, config_dt_tm)
...
class Tc15_Validate_Overall_Stack_Mode_Push_From_Custom_Group(aetest.Testcase):
    ...
    def Tc15_Validate_Overall_Stack_Mode_Push_From_Custom_Group(self, driver, ir510_eids ,cgmesh_pan_id, stackmd_exp_str, cus_fw_groupname,
                                                                stackmd_typ_init_exp, stackmd_typ_fin_exp, def_fw_groupname, stack_node_cnt1, 
        ...
        try:
            ...
            log.info("Verify Push stack mode in \'%s\' group" % cus_fw_groupname)
            temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, stackmd_typ_exp, stackmd_typ_init_exp,
                                                 stackmd_typ_fin_exp, pshstack_msg_exp, stack_node_cnt1)  
            if temp:
            ...
            log.info("Verify Push stack mode in \'%s\' group" % def_fw_groupname)
            temp = Push_And_Verify_The_StackMode_In_Custom_Group(driver, cgmesh_pan_id, stackmd_typ_exp, stackmd_typ_fin_exp, stack_node_cnt1)
            if temp:
...
class Tc16_Validate_Overall_Stack_Time_Push_From_Custom_Group(aetest.Testcase):
    ...
    def Tc16_Validate_Overall_Stack_Time_Push_From_Custom_Group(self, driver, nms_server, ir510_eids, cus_fw_groupname, def_fw_groupname,
                                                                      stktm_typ_init_exp, stktm_typ_fin_exp, cgmesh_pan_id, stackmd_exp_str, stack_node_cnt1,
        ...
        try:
            ...
            temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, stktm_typ_exp, stktm_typ_init_exp, stktm_typ_fin_exp, pshstack_tme_msg_exp, stack_node_cnt1)
            if temp:
                log.info("Overall Scheduled Stack mode time is Successfull in \'%s\'" % cus_fw_groupname)
            else:
                self.failed(banner("Overall Scheduled Stack mode time Failed in \'%s\'" % cus_fw_groupname))
            ...
            temp = Push_And_Verify_The_StackMode_In_Custom_Group(driver, cgmesh_pan_id, stktm_typ_exp, stktm_typ_fin_exp, stack_node_cnt1)
            if temp:
                log.info("Overall Scheduled Stack mode time is Successfull in \'%s\'" % def_fw_groupname)
            else:
                self.failed(banner("Overall Scheduled Stack mode time Failed in \'%s\'" % def_fw_groupname))
...
class Tc17_Validate_Overall_Stackmode_Time_Cancel_From_Custom_Group(aetest.Testcase):
    ...
    def Tc17_Validate_Overall_Stackmode_Time_Cancel_From_Custom_Group(self, driver, ir510_eids, cus_fw_groupname, def_fw_groupname,
                                                   ...
                                                   cancel_msg_exp, log_msg_cncl, cncl_stkmd_typ_init_exp, cncl_stkmd_typ_fin_exp):
        try:
            global driver_utils
            global auto_user_ws
            import datetime
            ...
            log.info("Verify Push stack mode time in \'%s\' group" % cus_fw_groupname)
            temp = Push_And_Verify_The_StackMode(driver, cgmesh_pan_id, cncl_stkmd_typ_exp, cncl_stkmd_typ_init_exp, 
                                                 cncl_stkmd_typ_fin_exp, cancel_msg_exp, stack_node_cnt1)
            ...
            log.info("Verify Push stack mode time in \'%s\' group" % def_fw_groupname)
            temp = Push_And_Verify_The_StackMode_In_Custom_Group(driver, cgmesh_pan_id, cncl_stkmd_typ_exp, cncl_stkmd_typ_fin_exp, stack_node_cnt1)
...
class Tc20_Validate_Schedule_Reload_And_Switch_stack_time_At_The_Same_Time(aetest.Testcase):
    @aetest.test
    def Tc20_Validate_Schedule_Reload_And_Switch_stack_time_At_The_Same_Time(self, driver, cgmesh_pan_id, stackmd_exp_str, stackmd_typ_init_exp,
                                                                             stackmd_cfm_exp_str, stackmd_typ_exp, pshstack_msg_exp, stackmd_typ_fin_exp,
...
class Tc21_Validate_Cancel_Reload_Stackmode(aetest.Testcase):
    @aetest.test
    def Tc21_Validate_Cancel_Reload__Stackmode(self, driver, ir510_eids, cgmesh_pan_id, cncl_exp_str, cancel_exp_str, stack_node_cnt, 
                                               cncl_stkmd_typ_init_exp, cncl_stkmd_typ_fin_exp, cncl_stkmd_typ_exp, cancel_msg_exp, log_msg_cncl):
        try:
            global driver_utils
            global auto_user_ws
            import datetime
